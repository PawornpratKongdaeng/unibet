...
export default function AdminBankSettings() {
  // ดึงข้อมูลมาแสดงผล (ถ้า Backend ส่ง 404 มา bankData จะเป็น null)
      ...
      try {
        ...
        const res = await apiFetch("/admin/config/bank", { 
          ...
          headers: { 
            ...
            "Authorization": `Bearer ${token}` // ✅ ส่ง Token ไปด้วยเพื่อแก้ 401
          },
        ...
        if (res.ok) {
          ...
          Swal.fire({ icon: 'error', title: 'Unauthorized', text: 'คุณไม่มีสิทธิ์จัดการส่วนนี้ หรือ Session หมดอายุ' });
        } else {
          Swal.fire({ icon: 'error', title: 'Failed', text: 'เกิดข้อผิดพลาดจาก Server' });
        }
      } catch (err) { 
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-12 p-4">
      ...
      <div className="flex justify-between items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 text-xs font-bold mt-2">จัดการบัญชีธนาคารสำหรับรับยอดฝาก</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white border border-slate-100 p-10 rounded-[3rem] shadow-sm relative overflow-hidden">
            <div className="space-y-6 relative z-10">
                <InputGroup icon={<Landmark size={18}/>} label="Bank Name" value={form.bank_name} placeholder="เช่น KASIKORN" onChange={(v) => setForm({...form, bank_name: v})} />
                <InputGroup icon={<User size={18}/>} label="Account Name" value={form.account_name} placeholder="ชื่อเจ้าของบัญชี" onChange={(v) => setForm({...form, account_name: v})} />
                <InputGroup icon={<CreditCard size={18}/>} label="Account Number" value={form.account_number} placeholder="000-0-00000-0" isNumeric onChange={(v) => setForm({...form, account_number: v})} />
            </div>
            <button type="submit" className="w-full bg-emerald-600 text-white font-black py-6 rounded-2xl mt-8 hover:bg-slate-900 transition-all">
               SAVE CONFIGURATION
            </button>
          </div>
        </form>
        ...
        {/* Live Preview */}
        <div className="bg-slate-900 p-12 rounded-[3.5rem] shadow-2xl flex flex-col justify-between text-white min-h-[350px]">
          ...
          <div>
            ...
            <p className="text-5xl font-black italic tracking-tighter break-all">{form.account_number || "000-0-0000-0"}</p>
          </div>
          <div className="border-t border-white/5 pt-8 flex justify-between">
            <div>
...
// Input Component
function InputGroup({ icon, label, value, placeholder, isNumeric, onChange }: any) {
  return (
    <div className="space-y-3">
      ...
      <input 
        ...
        className={`w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-5 text-slate-900 outline-none focus:border-emerald-500 transition-all font-black italic ${isNumeric ? 'text-3xl text-emerald-600' : 'text-lg'}`}
      />
    </div>
  );
}